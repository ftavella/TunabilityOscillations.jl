function test_model_creation(networks)
  """
  This test evaluates whether the function create_model creates a model with the correct number
  of species, parameters, and reactions.  Additionally, this test simulates that model and 
  corroborates that solutions are periodic by using previously simulated networks. The information 
  of those networks is stored in the files oscillatory_parameter_sets_*.jld2
  """
  for (name, connectivity) in networks
    ...
    for (idx, p_set) in enumerate(eachrow(p_sample))
        ...
        prob = remake(prob, p=ode_params, u0=initial_state, tspan=[0.0, 5*period])
        sol = solve(prob, hparams["solver"], abstol=1e-7, 
                        reltol=1e-4, maxiters=hparams["maxiters"])
        
        # Test that solution is periodic within 2% error
        @test 100*mean(abs.(sol[1] - sol[end])) < 2
    end
  end
end
